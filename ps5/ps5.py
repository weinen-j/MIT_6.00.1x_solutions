...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in lower:
            u = lower.index(i)
            d.update([(lower[u], lower[(u + shift) % 26])])
        for i in upper:
            u = upper.index(i)
            d.update([(upper[u], upper[(u + shift) % 26])])
        return d
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        return score.index(maximum), super().apply_shift(score.index(maximum))
